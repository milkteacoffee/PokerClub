...
一个**单文件、零依赖、离线可玩**的单机小游戏合集：德州扑克、21点、炸金花、猜骰子。双击 `index.html` 即可游玩；线上沿用 GitHub Pages：https://milkteacoffee.github.io/dezhou/ 。无真人联机、后端、充值或现金兑现，金币和积分都是本地虚拟数据。

## 当前规则 v15（优先于下方历史功能说明）

- 新档500金币，同浏览器最多3个存档；v14原位迁移保留金币、历史段位与已领取奖励。旧门票归档为 `legacyEntryCredits`，不转换成可兑换积分。
- 四个游戏均通过大厅 → 自由匹配/独立排位 → 新手/普通/困难/冠军进入。三个小游戏另有免费练习，不计任务成就。
- 金币钱包共用；四游戏 `games[gameId]` 各自记录可兑换积分、段位分、排位战绩、任务指标及各难度完成次数。
- 每100金币可兑换本游戏1积分，反向亦然；不能跨游戏转积分，兑换不改变段位。
- 排位不收门票、无10000金币门槛，独立比赛筹码每局重置，不兑换、不参与金币输赢。
- 新手/普通/困难/冠军排位净赢分别 +1/+2/+3/+4 可兑换积分及 +12/+16/+20/+26 段位分；净输分别 -1/-2/-3/-4 积分及 -5/-7/-9/-12 段位分；平局不变、最低0。离开/刷新未完局记负，升段不额外送金币。
- 日常5项共100金币，周常25项共910金币；1000项成就四馆各250项，沿用原总奖励预算。中途放弃不计完成任务。
- 骰子支持3颗/5颗；3骰小4–10大11–17，5骰小6–17大18–29；全部相同才是豹子，大小均输。可同轮押大小、总点数、任意豹子和指定豹子，多项分别结算，重复押法合并。
- 骰子赔率为含本金返还倍数，大小2倍，其它按 `floor(0.95 × 全部结果数 / 命中数)`；页面显示概率与返还率，不改随机结果。
- 小游戏单轮投入上限依难度为100/500/2000/10000；21点按公平牌组与软17停牌，难度改变额度及提示而不操纵牌；炸金花AI策略逐档增强。
- 新版兑换、小游戏资金/结算、德州终局金币及成长、排位结算使用同次持久化与失败回滚。保存失败时保留重试入口，不重复派奖；本地存档不是防作弊钱包，浏览器存储被清除后不可恢复。

### 回归测试

使用 Node.js 22+，需要 `jsdom`（通过 `NODE_PATH` 指定依赖目录）。依次运行 `tests/test.js`、`tests/dom.test.js`、`tests/lobby.test.js`、`tests/economy-v2.test.js`、`tests/arcade.test.js`、`tests/progression-v15.test.js`。新增专项覆盖独立资产、迁移、真实存储失败、刷新恢复、德州真实结算及五骰多押。

发布方式：通过 SSH 将验证后的提交推送 `origin/main`，由现有 GitHub Pages 自动部署；发布后比对线上 HTML 与本地源码。`outputs/` 是本地验收截图和中间材料，不纳入发布提交。

---

以下为历史功能说明，其中场次、任务、段位经济以以上 v15 规则和当前界面为准。
...
### 任务与成就
- 21 项新手任务（全清约 8,250 🪙）、9 选 5 的每日任务、16 项成就（含皇家同花顺、五连胜、战胜牌王）
