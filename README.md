...
### 程序化 BGM（零音频文件）
用 WebAudio **实时合成**背景音乐，不依赖任何 mp3/ogg，所以离线也能播、仓库里也不会有二进制资源。

- **6 首曲目**：大厅（悠闲爵士 ii-V-I-VI）、简单场（明亮轻快）、普通场（稳定律动）、困难场（小调紧张）、冠军场（和声小调压迫感）
- 每首曲子 = **4 小节和声进行 + 16 分音符旋律 + 四分音符贝斯 + 闭镲打击**
- **lookahead 调度器**：每 25ms 检查一次，提前 150ms 排程，避免 `setTimeout` 抖动导致节奏不稳
- **时钟跳变保护**：切后台再回来时若音频时钟大幅跳跃，会重新对齐而不是一次性补发成百上千个音符节点（这条有专门的单元测试覆盖）
...
- 音效与音乐各有独立开关
- 音频图：`osc → [sfxGain / musicGain] → masterGain → destination`，音乐总线额外压低到 34%，避免盖住音效
...
覆盖内容（**93 项断言**）：HTML 结构完整性、脚本无未捕获错误、大厅→场次→牌桌全流程点击、
...
```
tests/test.js    → 通过 136 / 失败 0   （easy / normal / hard / champion 四档均通过）
tests/dom.test.js → 通过 93 / 失败 0
```
