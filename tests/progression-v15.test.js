...
test('排位只有一种模式：对手强度随小段位平滑爬升',()=>{var s=w.profile15('holdem');
  ...
  assert.equal(at(0),'easy','青铜 III');
  assert.equal(at(668),'easy','青铜 I');
  assert.equal(at(1668),'easy','白银 I');
  assert.equal(at(2000),'normal','黄金 III');
  assert.equal(at(2668),'normal','黄金 I');
  assert.equal(at(3000),'normal','铂金 III');
  assert.equal(at(3668),'normal','铂金 I');
  assert.equal(at(4000),'hard','钻石 III');
  assert.equal(at(4668),'hard','钻石 I');
  assert.equal(at(5000),'hard','大师 III');
  assert.equal(at(5668),'champion','大师 I');
  assert.equal(at(6000),'champion','传奇 III');
