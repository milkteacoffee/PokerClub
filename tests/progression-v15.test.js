...
let passed=0;
function fresh(game='holdem',mode='coins',difficulty='easy') {w.G.active=false;w.G.players=[];w.G.quickRetry=null;w.G.rankRetry=null;w.Arcade.round=null;w.player.rankedPending=null;w.player.arcade.pending=null;w.createNewSaveAt(1);w.hubGame=game;w.App.mode=mode==='ranked'?'ranked':'quick';w.App.difficulty=difficulty;w.Arcade.mode=mode;w.Arcade.difficulty=difficulty;w.Arcade.diceCount=3;if(mode==='ranked')w.profile15(game).rankPoints=w.RANKED_ENTRY_MIN;/* 排位需要满 100 积分才能进场 */if(game!=='holdem')assert(w.openArcade(game));refundFee();}
/* 门票行为由 entry-exit.test.js 专项覆盖，这里退还以便断言牌局本身 */
...
test('段位晋升同时奖励金币与积分，且只发未领过的新段位',()=>{var s=w.profile15('holdem');
  ...
  s.rankPoints=w.RANKS[1].min;assert.deepEqual(w.grantTierReward15(s),{tier:1,coins:500,points:5});
  ...
  s.rankPoints=0;w.grantTierReward15(s);s.rankPoints=w.RANKS[1].min;assert.equal(w.player.coins,c0+500,'掉段回升到已领段位不补发');
  s.rankPoints=w.RANKS[4].min;var p=w.grantTierReward15(s);assert.equal(p.tier,4);
  assert.equal(p.coins,1200+2500+5000,'第3+4+5档一次补齐');assert.equal(p.points,12+25+50);});
test('排位只有一种模式：对手强度随小段位平滑爬升',()=>{var s=w.profile15('holdem');
  const at=(pts)=>{s.rankPoints=pts;return w.rankedDifficultyFor('holdem');};
  assert.equal(at(0),'easy','牌桌新人 III');
  assert.equal(at(668),'easy','第1档 I');
  assert.equal(at(1668),'easy','第2档 I');
  assert.equal(at(2000),'normal','老练牌手 III');
  assert.equal(at(2668),'normal','第3档 I');
  assert.equal(at(3000),'normal','心理读牌师 III');
  assert.equal(at(3668),'normal','第4档 I');
  assert.equal(at(4000),'hard','气场宗师 III');
  assert.equal(at(4668),'hard','第5档 I');
  assert.equal(at(5000),'hard','牌桌枭雄 III');
  assert.equal(at(5668),'champion','第6档 I');
  assert.equal(at(6000),'champion','德州王牌 III');
  assert.equal(at(9999),'champion','封顶后仍为冠军级');});
