...
test('称号表结构完整：ID 唯一、67 个、均带条件与图标', () => {
  const ids = w.TITLES.map(t => t.id);
  assert.equal(new Set(ids).size, ids.length, 'ID 应唯一');
  assert.equal(w.TITLES.length, 67);   /* 2026-09-17 富化 +20 */
  w.TITLES.forEach(t => {
...
test('称号面板渲染全部条目，已解锁可佩戴、未解锁隐藏名称', () => {
  ...
  assert.ok(box.textContent.includes('已解锁 1 / 67'), '应显示进度，实际：' + box.textContent.slice(0, 40));
...
test('称号页签切换与待解锁徽标', () => {
  ...
  assert.equal(badge.textContent, '67');
...
test('全部解锁后徽标归零', () => {
  ...
  assert.ok(doc.getElementById('titleList').textContent.includes('已解锁 67 / 67'));
});
