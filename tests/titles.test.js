...
test('称号面板渲染全部条目，已解锁可佩戴、未解锁隐藏名称', () => {
  ...
  assert.ok(box.textContent.includes('已解锁 1 / 39'), '应显示进度，实际：' + box.textContent.slice(0, 40));
...
test('称号页签切换与待解锁徽标', () => {
  ...
  assert.equal(badge.textContent, '39');
...
test('全部解锁后徽标归零', () => {
  ...
  assert.ok(doc.getElementById('titleList').textContent.includes('已解锁 39 / 39'));
});
