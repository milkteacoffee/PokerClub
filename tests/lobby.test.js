...
(async () => {
  ...
  try {
    ...
    check(_navBtns === 11, '大厅功能入口 11 个（底栏 6 主入口 + 次要 5，含残局，实测 ' + _navBtns + '）');
    ...
    for (const id of ['lbQStats', 'lbQHelp', 'lbPuzzle']) check(d.querySelector('.lobby-topbtns').contains(d.getElementById(id)), '次要入口在大厅顶部: ' + id);
