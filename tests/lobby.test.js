...
(async () => {
  ...
  try {
    ...
    check(_navBtns === 12, '大厅功能入口 12 个（底栏 6 主入口 + 次要 6，含残局/锦标赛，实测 ' + _navBtns + '）');
    ...
    for (const id of ['lbQStats', 'lbQHelp', 'lbPuzzle', 'lbTournament']) check(d.querySelector('.lobby-topbtns').contains(d.getElementById(id)), '次要入口在大厅顶部: ' + id);
